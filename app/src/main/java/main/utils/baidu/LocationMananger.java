...
public class LocationMananger {
    ...
    private BDLocation desLocation;//含详细地址的定位
    ...
    public  static LocationMananger  getInstance(Context context){
        if (null==locationMananger ){
            locationMananger = new LocationMananger(context);
        }
        else {
            locationMananger.setContext(context);
     ...
     * @param scanSpan 请求间隔
     */
    public LocationMananger(Context context,int scanSpan) {
        this.context = context;
...
//设置打开自动回调位置模式，该开关打开后，期间只要定位SDK检测到位置变化就会主动回调给开发者，该模式下开发者无需再关心定位间隔是多少，定位SDK本身发现位置变化就会及时回调给开发者
            ...
            locationOption.setOpenGps(true); // 打开手机GPS
...
//开始定位
    ...
    public class MyLocationListener extends BDAbstractLocationListener {
        ...
        public void onReceiveLocation(BDLocation location){
            ...
            if (location.getLocationDescribe()!=null&&!location.getLocationDescribe().equals("")){
              desLocation = location;
            }
            if (onReceiveLocationListener != null){
                onReceiveLocationListener.onReceiveLocation(location);
            }
//            if (scanSpan==0&&location.getLocationDescribe()!=null&&!location.getLocationDescribe().equals("")){
//                locationClient.stop();
//            }
